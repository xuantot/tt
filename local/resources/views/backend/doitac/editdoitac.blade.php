        ...
        <form  method="post" enctype="multipart/form-data" action="">
          ...
          <div class="box-body">
            <div class="form-group">                          
              ...
              <input type="text" class="form-control" id="vd_name" name="vd_name" placeholder="Đối Tác" value="{{$doitac->vd_name}}">
            </div>
            <div class="form-group">
                <label>Ảnh (169 : 72 px)</label> 
              @if($errors->has('vd_pic'))<p class='text-red'><strong>{{$errors->first('vd_pic')}}</p></strong>@endif     
              <input type="file" class="form-control" id="form_pic" name="vd_pic" class="form-control" style="display:none" onchange="changeImg(this)" value="{{$doitac->vd_pic}}">
              <img id="avatar" class="thumbnail cursorpoint"width="169px" height="72px" src="public/image/{{$doitac->vd_pic}}">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="vd_link" name="vd_link" placeholder="ita/khoa-hoc-hot" value="{{$doitac->vd_link}}">
              </div>
            <div class="form-group">
              <label>Sắp Xếp</label>
              @if($errors->has('vd_order'))<p class='text-red'><strong>{{$errors->first('vd_order')}}</p></strong>@endif   
              <input type="text" class="form-control" id="vd_order" name="vd_order" placeholder="" value="{{$doitac->vd_order}}">
            </div>           
          </div>
          <!-- /.box-body -->
